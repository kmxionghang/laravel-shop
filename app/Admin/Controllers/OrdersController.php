...
//        }
    ...
    public function handleRefund(Order $order, HandleRefundRequest $request)
    {
        ...
        if ($request->input('agree')) {
            ...
            $this->_unRefundOrder($order, $request->input('reason'));
        }

        return $order;
    }

    protected function _unRefundOrder($order, $refund_disagree_reason)
    {
        // 将拒绝退款理由放到订单的 extra 字段中
        $extra = $order->extra ?: [];
        $extra['refund_disagree_reason'] = $refund_disagree_reason;
        // 将订单的退款状态改为未退款
        $order->update([
